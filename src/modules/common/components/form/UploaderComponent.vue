  ...
  <q-card class="q-my-sm">
    <q-card-section class="bg-secondary q-pa-sm">
      <div class="row items-center no-wrap">
        <div class="col">
          <div class="q-uploader__title text-white">Images</div>
        </div>
        <div class="col-auto">
          <q-btn
            color="white"
  ...
  setup(props, {emit}) {
    ...
    const onFileChange = (e) => {
      ...
      if (files.length + localImages.length > props.maximumQuantity) {
        et.notify({
          message: `You cannot add more than ${props.maximumQuantity} images !`,
          color: 'negative',
          icon: 'error'
        })
        return
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (file.size > props.maxSize * 1024 * 1024) {
          et.notify({
            message: `Images must be less than or equal to ${props.maxSize} MB!`,
            color: 'negative',
            icon: 'error'
          })
          continue
